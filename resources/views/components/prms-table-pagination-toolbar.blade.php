...
@php
    ...
    $showControls = PrmsTablePagination::needsControls($total, $perPage);
@endphp
...
@if ($showControls)
    <div {{ $attributes->merge(['class' => 'prms-table-pagination-toolbar d-flex flex-nowrap align-items-center justify-content-between gap-2 py-2 px-3 border-bottom overflow-auto']) }}>
        <p class="small text-muted mb-0 text-nowrap">
        ...
        <form method="GET" action="{{ url()->current() }}" class="d-flex flex-nowrap align-items-center gap-2 mb-0">
